...
public class ParkingSessionControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    private void createAndSaveParkingSession(String vehicleNumber, VehicleType vehicleType, ParkingSessionStatus status, Instant entryTime, Instant exitTime, Double totalAmount, String lotName, int slotNumber) {
        // Create ParkingLot
    ...
    void testGetActiveSessions_WithResults() throws Exception {
        // Arrange
        createAndSaveParkingSession("ABC-123", VehicleType.CAR, ParkingSessionStatus.ACTIVE, Instant.now().minus(Duration.ofHours(1)), null, null, "Lot A", 1);
    ...
    void testGetSessionHistory_WithResults() throws Exception {
        // Arrange
        createAndSaveParkingSession("DEF-456", VehicleType.CAR, ParkingSessionStatus.COMPLETED, Instant.now().minus(Duration.ofHours(2)), Instant.now().minus(Duration.ofHours(1)), 20.0, "Lot B", 2);
    ...
    void testGetActiveSessions_PaginationAndSorting() throws Exception {
        ...
        IntStream.range(0, 5).forEach(i ->
                createAndSaveParkingSession("VEH-" + (5 - i), VehicleType.CAR, ParkingSessionStatus.ACTIVE, Instant.now().minus(Duration.ofMinutes(i)), null, null, "Lot C", i + 1)
        );
    ...
    void testGetSessionHistory_PaginationAndSorting() throws Exception {
        // Arrange
        IntStream.range(0, 5).forEach(i ->
                createAndSaveParkingSession("HIST-" + (5 - i), VehicleType.BIKE, ParkingSessionStatus.COMPLETED, Instant.now().minus(Duration.ofHours(5 - i)), Instant.now().minus(Duration.ofHours(4 - i)), 10.0 + i, "Lot D", i + 1)
        );
    ...
    void testGetActiveSessions_PageBeyondBounds() throws Exception {
        // Arrange
        createAndSaveParkingSession("BOUND-1", VehicleType.CAR, ParkingSessionStatus.ACTIVE, Instant.now().minus(Duration.ofMinutes(10)), null, null, "Lot E", 1);
    ...
    void testGetSessionHistory_PageBeyondBounds() throws Exception {
        // Arrange
        createAndSaveParkingSession("BOUND-2", VehicleType.CAR, ParkingSessionStatus.COMPLETED, Instant.now().minus(Duration.ofHours(3)), Instant.now().minus(Duration.ofHours(2)), 30.0, "Lot F", 1);
