...
public class ParkingLotControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    void testGetAllParkingLots_NonPaginated_Success() throws Exception {
        ...
        assertThat(lots).extracting(ParkingLotDto::getName).containsExactlyInAnyOrder("Active Lot 1", "Active Lot 2");
    }
    ...
    void testDeleteParkingLot_Failure_WithActiveReservation() throws Exception {
        ...
        reservationRepository.save(Reservation.builder().parkingSlot(slot).vehicle(vehicle).status(ReservationStatus.ACTIVE).reservationTime(Instant.now()).expirationTime(Instant.now().plus(Duration.ofHours(1))).build());
